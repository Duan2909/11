...
# 載入自訂字型
font_path = "font.otf"  # 假設字型檔案在同一目錄
try:
    font_manager.fontManager.addfont(font_path)
    # 動態獲取字型名稱
    font = font_manager.FontProperties(fname=font_path)
    font_name = font.get_name()
    plt.rcParams['font.family'] = font_name  # 使用字型檔案的實際名稱
    plt.rcParams['axes.unicode_minus'] = False  # 確保負號正確顯示
except FileNotFoundError:
    st.error("字型檔案 font.otf 未找到！請確保檔案存在於專案目錄。")
    plt.rcParams['font.family'] = 'sans-serif'  # 退回預設字型
except Exception as e:
    st.error(f"載入字型失敗：{str(e)}。請檢查 font.otf 是否為有效字型檔案。")
